...
config.head_name = "BoundaryFace"               # head名 (partial_fc,BoundaryFace,exBoundaryFace)
config.resume = False                               # 学習を再開するか (True/False)
config.output = "/workspace/Output/casia_r50_BoundaryFace"         # モデルとログの保存先 (例: "/workspace/runs/my_run")
...
config.boundary_epoch_start = 35            # ノイズ除去処理を開始するエポック (デフォルト: 7)
...
config.milestones = [18, 28, 35, 38]        # MultiStepLR用のマイルストーンエポックリスト (schedulerが"Multistep"の場合に使用)
config.gamma = 0.1              # 変更時の減衰率


# --- 6. ClearML設定 ---
#task_name = f"{hostname}-{head_name}-{backbone}-{other_tags}"    のようになる
config.project_name = "Suzuki_project/sotuken"          # プロジェクト名
config.host_name = "pkh3212a"           # サーバー名
config.backbone_name = "se-iResNet50"            # backbone名
config.other_tags = "casia"              # その他タグ
