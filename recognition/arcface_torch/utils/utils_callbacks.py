...
class CallBackVerification(object):
    ...
    def __init__(self, val_targets, rec_prefix, summary_writer=None, image_size=(112, 112), wandb_logger=None, clearml_logger=None):
        ...
        self.clearml_logger = clearml_logger  #ClearMLに出力するためのオブジェクト
            ...
            if self.clearml_logger:
                try:
                    self.clearml_logger.report_scalar(
                        'Eval', # グラフのタイトル
                        self.ver_name_list[i], # シリーズ名 (例: 'lfw', 'cfp_fp')
                        iteration=global_step, 
                        value=acc2 # Accuracy-Flip の値
                    )
                    self.clearml_logger.report_scalar(
                    'Eval Acc Highest',  # ★ 新しいグラフタイトル
                    self.ver_name_list[i],  # シリーズ名 (例: 'lfw')
                    iteration=global_step,
                    value=self.highest_acc_list[i]  # ★ 最高の精度
                    )
                except Exception as e:
                    # logging をインポートしていない場合は print を使う
                    print(f'[Warning] ClearML report_scalar failed for Eval: {e}')
    ...
    def init_dataset(self, val_targets, data_dir, image_size):
        ...
        # 実際の検証データセットがあるルートパスを定義
        val_root_path = "/workspace/TestDataSets/test/" 

        for name in val_targets:
            
            # config.val_targets の名前 (lfw, cfp_fp, agedb_30) に応じて
            # 正しいファイルパスを組み立てる
            if name == 'lfw':
                ...
                path = os.path.join(val_root_path, "CALFW", "calfw.bin")
            elif name == 'cplfw':
                path = os.path.join(val_root_path, "CPLFW", "cplfw.bin")
            else:
                # 従来の動作 (config.rec と同じ場所を探す)
            ...
            if os.path.exists(path):
                ...
                # もしファイルが見つからなかった場合に警告を出す
                logging.warning(f"Verification file not found, skipping: {path}")
